...
namespace ns3 {
  NS_LOG_COMPONENT_DEFINE ("MyAlgorithmProactive");

  MyAlgorithmProactive::MyAlgorithmProactive (){
  ...
  bool MyAlgorithmProactive::loadConfig(std::string filename){
...
void MyAlgorithmProactive::initFuncMap(){
...
void MyAlgorithmProactive::loadTopo(){
    NS_LOG_FUNCTION("this");
...
void MyAlgorithmProactive::loadRequest(){
    NS_LOG_FUNCTION("this");
...
void MyAlgorithmProactive::loadPredictRequest(){
    NS_LOG_FUNCTION("this");
...
std::map<int, int> MyAlgorithmProactive::genZipfNum(int num_values, float alpha, int max_num){
  NS_LOG_FUNCTION("this");
...
bool MyAlgorithmProactive::createRequest(int timeSlot, int funcType, int ingressID, Request &r, bool isNotPredicted, bool isNotUsed){
    NS_LOG_FUNCTION(this);
...
void MyAlgorithmProactive::createRequestInSlot(int timeSlot, int funcType, ReqOnNodes ron, ReqOnNodes predRon){
    NS_LOG_FUNCTION(this);
    ...
    for(int i = 0; i < ron.numVector.size(); i++){
        ...
        if (difference > 0){
            NS_LOG_ERROR("request num difference is " << difference);
        }
        ...
        for (int i = 0; i < rNum; i++){
            ...
            if(difference > i){
                ...
                isNotUsed = true;
                NS_LOG_ERROR("unused num " << unused_count);
                unused_count++;
...
void MyAlgorithmProactive::createRequests(){
...
void MyAlgorithmProactive::readRequests(){
    // std::map<int, int> req_map = genZipfNum(125, 0.5, 519);
...
void MyAlgorithmProactive::readPredictRequests(){
    // std::map<int, int> req_map = genZipfNum(125, 0.5, 519);
...
//return meter
float MyAlgorithmProactive::distance(int node_1, int node_2){
    NS_LOG_FUNCTION(this);
...
DistSlice MyAlgorithmProactive::sortPhyNodes(Request r){
    NS_LOG_FUNCTION(this);
...
Function MyAlgorithmProactive::getIdleFunction(int phynodeID, int funcType, int &index){
    NS_LOG_FUNCTION(this);
...
void MyAlgorithmProactive::placeToNeighbour(Request &r, Function function, int index, int phyNodeID){
    NS_LOG_FUNCTION(this);
...
bool MyAlgorithmProactive::deployToNeighbour(DistSlice ds, Request &r){
    NS_LOG_FUNCTION(this);
...
void MyAlgorithmProactive::createToCurrent(Request &r){
    NS_LOG_FUNCTION(this);
    bool succFlag = true;
...
void MyAlgorithmProactive::placeToCurrent(Request &r, Function f, int index){
    NS_LOG_FUNCTION(this);
    //update the freq
...
void MyAlgorithmProactive::deployRequest(Request &r){
    NS_LOG_FUNCTION(this);
...
void MyAlgorithmProactive::updateCache(){
    NS_LOG_FUNCTION(this);
...
void MyAlgorithmProactive::deployRequests(){
    NS_LOG_FUNCTION(this);
...
void MyAlgorithmProactive::init(){
...
void MyAlgorithmProactive::printResult(std::string filename){
    NS_LOG_FUNCTION(this);
    ...
    for(int i=1; i<= m_request_map.size(); i++){
        if(m_request_map.rsMap.find(i) != m_request_map.rsMap.end()){
            ...
            for(auto it = it_map->second.rs.begin(); it != it_map->second.rs.end(); it++){
                ...
                if((*it).isColdStart == true && (*it).isNotPredicted){
...
void MyAlgorithmProactive::printResult_no_1(std::string filename){
    NS_LOG_FUNCTION(this);
    //result [time slot, id, type, linkdelay, processingdelay, coldstartdelay, iscoldstart, total delay]
    std::vector<float> result;
    float totalDelay = 0;
...
void MyAlgorithmProactive::genTraffic(int time_slot, int time_slot_num){
    NS_LOG_FUNCTION(this);
...
void MyAlgorithmProactive::scheduleRequests(float beta_input, float reduFactor_input){
    ...
    std::string filename = "result/proactive-my-result-"+ beta +"-" + reducefactor+ ".csv";
...
void MyAlgorithmProactive::calculateMem(){
