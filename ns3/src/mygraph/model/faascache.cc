...
namespace ns3 {
  NS_LOG_COMPONENT_DEFINE ("Faascache");
  ...
  bool Faascache::loadConfig(std::string filename){
...
std::map<int, int> Faascache::genZipfNum(int num_values, float alpha, int max_num){
  NS_LOG_FUNCTION("this");
...
bool Faascache::createRequest(int timeSlot, int funcType, int ingressID, Request &r){
    NS_LOG_FUNCTION(this);
...
void Faascache::createRequestInSlot(int timeSlot, int funcType, ReqOnNodes ron){
    NS_LOG_FUNCTION(this);
...
//return meter
float Faascache::distance(int node_1, int node_2){
    NS_LOG_FUNCTION(this);
...
DistSlice Faascache::sortPhyNodes(Request r){
    NS_LOG_FUNCTION(this);
...
Function Faascache::getIdleFunction(int phynodeID, int funcType, int &index){
    NS_LOG_FUNCTION(this);
...
void Faascache::createNew(Request &r, int phyNodeID){
    NS_LOG_FUNCTION(this);
    bool succFlag = true;
    Function f;
    int count = 0;
    r.function.activePriority(m_clock, m_functionfreq);
    ...
    if(r.function.size > m_topo.get(phyNodeID).mem){
        while (count < 1) {
            ...
            float p2 = m_cm.getLowestPriority(phyNodeID);

            if (p2 < r.function.priority){
                NS_LOG_INFO("------lowest priority------");
                succFlag = m_cm.deleteLowestPriority(phyNodeID, f, m_functionfreq);
...
void Faascache::deployRoundRobin(Request &r){
    NS_LOG_FUNCTION(this);
...
void Faascache::createToCurrent(Request &r){
    NS_LOG_FUNCTION(this);
    bool succFlag = true;
...
void Faascache::placeToCurrent(Request &r, Function f, int index, int phyNodeID){
    NS_LOG_FUNCTION(this);
    //update the freq
...
//TODO: round robin
void Faascache::deployRequest(Request &r){
    NS_LOG_FUNCTION(this);
...
void Faascache::deployRequests(int timeslotnum){
    NS_LOG_FUNCTION(this);
...
void Faascache::printResult(std::string filename){
    NS_LOG_FUNCTION(this);
...
void Faascache::printResult_no_1(std::string filename){
    NS_LOG_FUNCTION(this);
    //result [time slot, id, type, linkdelay, processingdelay, coldstartdelay, iscoldstart, total delay]
    std::vector<float> result;
    float totalDelay = 0;
...
void Faascache::genTraffic(int time_slot, int time_slot_num){
    NS_LOG_FUNCTION(this);
...
void Faascache::scheduleRequests(float beta_input, float reduFactor_input){
    ...
    std::string filename = "result/faascache-result-"+ beta +"-" + reducefactor + ".csv";
