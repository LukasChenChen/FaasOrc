...
namespace ns3 {
   ...
   int lifeTimeHist_G = 29;

  NS_LOG_COMPONENT_DEFINE ("Hist");
  ...
  bool Hist::loadConfig(std::string filename){
...
void Hist::initFuncMap(int lifeTime){
   
    //processing time in ms, cold start time in seconds, size
    FunctionInfo fi = {350, 1, 55, lifeTime};
    m_funcInfoMap.add(1,fi);

    fi = {630, 2.1, 158, lifeTime};
    m_funcInfoMap.add(2,fi);

    fi = {200, 2.2, 332, lifeTime};
    m_funcInfoMap.add(3,fi);

    fi = {207, 2.3, 92, lifeTime};
    m_funcInfoMap.add(4,fi);

    fi = {500, 2.4, 30, lifeTime};
    m_funcInfoMap.add(5,fi);

    fi = {100, 2.5, 40, lifeTime};
    m_funcInfoMap.add(6,fi);

    fi = {150, 2.6, 50, lifeTime};
    m_funcInfoMap.add(7,fi);

    fi = {200, 2.7, 60, lifeTime};
    m_funcInfoMap.add(8,fi);

    fi = {175,2.8, 70, lifeTime};
    m_funcInfoMap.add(9,fi);

    fi = {125, 2.9, 80, lifeTime};
...
std::map<int, int> Hist::genZipfNum(int num_values, float alpha, int max_num){
  NS_LOG_FUNCTION("this");
...
bool Hist::createRequest(int timeSlot, int funcType, int ingressID, Request &r){
    NS_LOG_FUNCTION(this);
...
void Hist::createRequestInSlot(int timeSlot, int funcType, ReqOnNodes ron){
    NS_LOG_FUNCTION(this);
...
//return meter
float Hist::distance(int node_1, int node_2){
    NS_LOG_FUNCTION(this);
...
DistSlice Hist::sortPhyNodes(Request r){
    NS_LOG_FUNCTION(this);
...
Function Hist::getIdleFunction(int phynodeID, int funcType, int &index){
    NS_LOG_FUNCTION(this);
...
void Hist::placeToNeighbour(Request &r, Function function, int index, int phyNodeID){
    NS_LOG_FUNCTION(this);

    // function.activeLifeTime(lifeTimeHist_G);
...
bool Hist::deployToNeighbour(DistSlice ds, Request &r){
    NS_LOG_FUNCTION(this);
...
void Hist::createToCurrent(Request &r){
    NS_LOG_FUNCTION(this);
    bool succFlag = true;
...
void Hist::placeToCurrent(Request &r, Function f, int index){
    NS_LOG_FUNCTION(this);
    //renew the lifetime
    // f.activeLifeTime(lifeTimeHist_G);
...
void Hist::deployRequest(Request &r){
    NS_LOG_FUNCTION(this);
...
void Hist::updateCache(){
    ...
    for(auto it = m_cm.caches.begin(); it != m_cm.caches.end(); it++){
        for(auto it_fl = it->second.functionList.begin(); it_fl != it->second.functionList.end(); ){
            ...
            (*it_fl).minusLife();
            NS_LOG_LOGIC("minus life ");
            if((*it_fl).lifeTime < 1){
                NS_LOG_LOGIC("Function life time ends " << (*it_fl).type);
...
void Hist::printResult_no_1(std::string filename){
    NS_LOG_FUNCTION(this);
    //result [time slot, id, type, linkdelay, processingdelay, coldstartdelay, iscoldstart, total delay]
    std::vector<float> result;
    float totalDelay = 0;
...
void Hist::genTraffic(int time_slot, int time_slot_num){
    NS_LOG_FUNCTION(this);
