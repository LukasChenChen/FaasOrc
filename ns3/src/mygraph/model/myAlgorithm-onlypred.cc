...
namespace ns3 {
  NS_LOG_COMPONENT_DEFINE ("MyAlgorithmOnlypred");

  MyAlgorithmOnlypred::MyAlgorithmOnlypred (){
  ...
  bool MyAlgorithmOnlypred::loadConfig(std::string filename){
    ...
    std::string TopoName, RequestFile, predictRequestFile;
    ...
    if (myfile.is_open()) {
        ...
        while (getline (myfile, line)) {
            ...
            } else if (tmp_key == "predictRequestFile")
            {
                predictRequestFile = line.substr(pos+1);
                std::cout<< "predictRequestFile " << predictRequestFile << std::endl;
                break;
            }
    ...
    m_cfg.predictRequestFile = predictRequestFile;
...
void MyAlgorithmOnlypred::initFuncMap(){
...
void MyAlgorithmOnlypred::loadTopo(){
    NS_LOG_FUNCTION("this");
...
void MyAlgorithmOnlypred::loadRequest(){
    NS_LOG_FUNCTION("this");
...
void MyAlgorithmOnlypred::loadPredictRequest(){
    NS_LOG_FUNCTION("this");

    std::string line;

    std::ifstream myfile(m_cfg.predictRequestFile);

    NS_LOG_INFO("-----read predict Request file-----");
    ...
    if (myfile.is_open()) {
        ...
        {   
            ...
            NS_LOG_LOGIC("insert function type " << rowNum/2);
            m_pred_req_num.insert({ rowNum/2 ,reqNumMap});

            rowNum++;
           
        }
    }else{
        NS_LOG_ERROR("No such predicted result file ");
    }
...
std::map<int, int> MyAlgorithmOnlypred::genZipfNum(int num_values, float alpha, int max_num){
  NS_LOG_FUNCTION("this");
...
bool MyAlgorithmOnlypred::createRequest(int timeSlot, int funcType, int ingressID, Request &r, bool isNotPredicted, bool isNotUsed){
    NS_LOG_FUNCTION(this);
...
void MyAlgorithmOnlypred::createRequestInSlot(int timeSlot, int funcType, ReqOnNodes ron, ReqOnNodes predRon){
    NS_LOG_FUNCTION(this);
    ...
    for(int i = 0; i < ron.numVector.size(); i++){
        ...
        int predRnum = predRon.numVector[i];

        int difference = predRnum - rNum;

        rNum = (int)(rNum/m_cfg.ReduFactor);
        difference = (int)(difference/m_cfg.ReduFactor);
        NS_LOG_INFO("request num difference is " << difference);

        if (difference > 0){
            NS_LOG_INFO("request num difference is " << difference);
        }

        int unused_count = 0; //number of used containers
         //create rNum requests
        for (int i = 0; i < rNum; i++){

            Request r;
            bool isNotPredicted = false;//if this is true means we did not predict this one, and hence cold start
            bool isNotUsed = false;
            if(difference > i){
                isNotPredicted = true;
            }else if (difference > 0 && unused_count <= abs(difference)){
                //if this true, means some are wasted
                isNotUsed = true;
                NS_LOG_INFO("unused num " << unused_count);
                unused_count++;
                
            }


            createRequest(timeSlot, funcType, nodeID, r, isNotPredicted, isNotUsed);
...
void MyAlgorithmOnlypred::createRequests(){

    for (auto it = m_rontt.numMap.begin(); it != m_rontt.numMap.end(); it++){
        int funcType = it->first;
        ReqOnNodesTime ront = it->second;
        if(m_prontt.numMap.find(funcType) == m_prontt.numMap.end()){
            NS_LOG_ERROR("cannot find such predictedrequestmap for func " << funcType);
            NS_LOG_ERROR("m_prontt size " << m_prontt.size());
            return;
        }
        ReqOnNodesTime predRont = m_prontt.numMap[funcType];
        //iterate over all time slot
        for(int i = 0; i < ront.numVector.size(); i++){
            //time slot start from 1
            createRequestInSlot(i+1, funcType, ront.numVector[i], predRont.numVector[i]);
        }
    }
...
void MyAlgorithmOnlypred::readRequests(){
    // std::map<int, int> req_map = genZipfNum(125, 0.5, 519);
...
//read the predict requests
void MyAlgorithmOnlypred::readPredictRequests(){
    // std::map<int, int> req_map = genZipfNum(125, 0.5, 519);
    ...
    std::ifstream myfile("config/requests-predict-" + std::to_string(m_cfg.Beta) +".csv");

    NS_LOG_INFO("-----read predcited zipf requests file-----");
    ...
    if (myfile.is_open()) {
        ...
        {   
         ...
         //when all finish add type 4
        if (!getline (myfile, line)){
            NS_LOG_LOGIC("add functype to predicted map " << funcType);
            m_prontt.add(funcType, ront);
        }
    }//end if
    else{
        NS_LOG_ERROR("cannot find predicted request file " );
    }
...
//return meter
float MyAlgorithmOnlypred::distance(int node_1, int node_2){
    NS_LOG_FUNCTION(this);
...
DistSlice MyAlgorithmOnlypred::sortPhyNodes(Request r){
    NS_LOG_FUNCTION(this);
...
Function MyAlgorithmOnlypred::getIdleFunction(int phynodeID, int funcType, int &index){
    NS_LOG_FUNCTION(this);
...
bool MyAlgorithmOnlypred::placeToNeighbour(Request &r, int phyNodeID){
    NS_LOG_FUNCTION(this);
...
bool MyAlgorithmOnlypred::deployToNeighbour(DistSlice ds, Request &r){
    NS_LOG_FUNCTION(this);
    ...
    for(int j = 1; j < ds.size(); j++){
        if(ds.vec[j].distance * m_cfg.LatencyPara < r.function.coldStartTime){
            ...
            //place to this node
            succFlag = placeToNeighbour(r, ds.vec[j].phyNodeID);
...
bool MyAlgorithmOnlypred::createToCurrent(Request &r){
    NS_LOG_FUNCTION(this);
...
void MyAlgorithmOnlypred::placeToCurrent(Request &r, Function f, int index){
    NS_LOG_FUNCTION(this);
    //update the freq
...
void MyAlgorithmOnlypred::deployRequest(Request &r){
    NS_LOG_FUNCTION(this);

    DistSlice ds = sortPhyNodes(r);

    bool succFlag = createToCurrent(r);

    if (succFlag == true){
        return;
    }else{
        // deployToNeighbour(ds, r);
    } 

}

void MyAlgorithmOnlypred::updateCache(){
    NS_LOG_FUNCTION(this);
...
void MyAlgorithmOnlypred::deployRequests(){
    NS_LOG_FUNCTION(this);
    ...
    for(int i=1; i<= m_request_map.size(); i++){
        ...
        m_topo.restore(m_cfg.MemCap);

        NS_LOG_ERROR("memoryLeft " << memoryLeft);
...
void MyAlgorithmOnlypred::printResult(std::string filename){
    NS_LOG_FUNCTION(this);
    ...
    for(int i=1; i<= m_request_map.size(); i++){
        if(m_request_map.rsMap.find(i) != m_request_map.rsMap.end()){
            ...
            for(auto it = it_map->second.rs.begin(); it != it_map->second.rs.end(); it++){
                ...
                if((*it).isNotUsed == true){
                    result.push_back(1.0);
                    m_unused_num++;
                    m_unused_mem += (*it).function.size;
                }else{
                    result.push_back(0.0);
                }
    ...
    numbers.push_back(m_unused_num);
    numbers.push_back(m_unused_mem/30/10); //averag mem unused on each node
...
void MyAlgorithmOnlypred::printResult_no_1(std::string filename){
    NS_LOG_FUNCTION(this);
    //result [time slot, id, type, linkdelay, processingdelay, coldstartdelay, iscoldstart, total delay]
    std::vector<float> result;
    float totalDelay = 0;
...
void MyAlgorithmOnlypred::genTraffic(int time_slot, int time_slot_num){
    NS_LOG_FUNCTION(this);
...
void MyAlgorithmOnlypred::scheduleRequests(float beta_input, float reduFactor_input){
    ...
    NS_LOG_INFO("Total init memory<< " << totalMem);

    initFuncMap();
    readRequests();
    readPredictRequests();//read the predict number of request
    createRequests();
    ...
    std::string filename = "result/onlypred-my-result-"+ beta +"-" + reducefactor+ ".csv";
...
void MyAlgorithmOnlypred::calculateMem(){
