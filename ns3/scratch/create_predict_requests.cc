...
#include "ns3/zipf.h"
#include "ns3/myAlgorithm-proactive.h"



using namespace ns3;

NS_LOG_COMPONENT_DEFINE ("create-predict");
...
{
   LogComponentEnable ("MyAlgorithmProactive", LOG_LEVEL_INFO);
   LogComponentEnable ("Zipf", LOG_LEVEL_INFO);

   int slotNum = 30;//how many slot we want
   

   MyAlgorithmProactive ma;
   ma.loadConfig("config/config.txt");
   ma.loadTopo();
   ma.initFuncMap();
   ma.loadPredictRequest();

   std::string fileName = "config/requests-predict-" + std::to_string(ma.m_cfg.Beta) +".csv";

    // Generate and output zipf random variables

    //for different type of requests
    for(auto const& typeRequests : ma.m_pred_req_num){
        ...
        for(int i = 1; i <= slotNum; i++){
                ...
                NS_LOG_LOGIC(" create predict Request---------");
