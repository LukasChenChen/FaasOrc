...
func (f *Function) calcHotscore(){
	count := functionfreq_G.get(f.Type)
	f.Hotscore = math.Pow(float64(2), float64(1-count))*float64(count)
}
...
type Config struct {
	...
	PredRequestFile string    `json:"PredRequestFile"`
...
func (c *Cache) sortList() {
    sort.SliceStable(c.FunctionList, func(i, j int) bool {
		return c.FunctionList[i].Hotscore < c.FunctionList[j].Hotscore
	})
}
...
func (cm *CacheMap) getLowestHotscore(phyNodeID int) float64{
	c := cm.Caches[phyNodeID]
...
//delete the function with lowest hotscore
func (cm *CacheMap) deleteLowestHotscore(phyNodeID int) (Function, bool) {
	c := cm.Caches[phyNodeID]
