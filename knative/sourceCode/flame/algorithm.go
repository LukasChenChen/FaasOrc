...
var m_current_node_index_G int
...
func initGlobal(){
    ...
    clock_G = 1
    requestCount_G = 0
    m_testbed_G = config_G.testBed
}

//assign a request to the current phynode with cached container
func placeToCurrent(requestPtr *Request, Function Function, i int, phyNodeID int){ 

    log.Println( "start placeToCurrent....")
    //put it in the active list
    requestPtr.Function.calcHotscore()
    activeFunctions_G.add(Function, phyNodeID)

	//delete it from the cached list
    cacheMap_G.delete(phyNodeID, i)

    deployNode, _ := getPhyNode(phyNodeID)
...
func createContainers(requestPtr *Request){
    ...
    count_G = count_G + 1
    //the count is also given here, to add count to the name
    requestPtr.Function.init(requestPtr.Function.PhyNode) // create new request only one case, on the current node

	service, err := createService(requestPtr.Function)

    requestPtr.update(requestPtr.Function, requestPtr.Function.PhyNode, true)
...
func createNew(requestPtr *Request, phyNodeID int){
    log.Println("Starting createNew......")
    succFlag := true
    var f Function
    count := 0
    requestPtr.Function.calcHotscore()
   
    if(requestPtr.Function.Size > topo_G.Nodes[phyNodeID].Mem){
        for count < 1 {
            count++

            p2 := cacheMap_G.getLowestHotscore(phyNodeID)

            if p2 < requestPtr.Function.Hotscore {
                log.Println("------lowest hotscore------")
                f ,succFlag = cacheMap_G.deleteLowestHotscore(phyNodeID);

                if succFlag == true{
                    //if success terminate the container
                   // terminate(f); end the traffic application
                   termContainers(f)
                   updateTopo("add", phyNodeID, f.Size)
                }
                ...
                if (requestPtr.Function.Size <= topo_G.Nodes[phyNodeID].Mem){
                    ...
                    p := topo_G.Nodes[phyNodeID]

                    if p.ID == 0{
                        return
                    }
                    
                    requestPtr.Function.PhyNode = p

                    createContainers(requestPtr)

                    // NS_ASSERT_MSG( phyNodeID <= 125 && phyNodeID > 0 , "phyNodeID smaller than 125");

                    activeFunctions_G.add(requestPtr.Function, phyNodeID)

                    updateTopo("minus", phyNodeID, requestPtr.Function.Size)

                    requestPtr.update(requestPtr.Function, p, true)
            ...
            log.Println("------no other option------");
                return
           }//endif
       }//end while
    }else{

        log.Println("------create hotscore------")
        requestPtr.Function.calcHotscore()
...
func deployRoundRobin(requestPtr *Request){
    log.Println("......Starting deployRoundRobin......")

    node_num := config_G.NodeNum
    //node_id 0-9 in knative
    if(m_current_node_index_G > node_num-1){
        m_current_node_index_G = 1
    }
    j := m_current_node_index_G

    index := -1
    
    f, index := cacheMap_G.getIdleFunction(j, requestPtr.Function.Type)

    log.Println("......deployRoundRobin node index", j)

    if index != -1 {
        placeToCurrent(requestPtr, f, index, j)
    }else{
        createNew(requestPtr, j)
    }

    m_current_node_index_G++
...
func deployRequest(requestPtr *Request){
    ...
    deployRoundRobin(requestPtr)

    collectResult(requestPtr)
...
func sendResult(r Request) error{
    ...
    {
        log.Printf("sendResult: HTTP status code", resp.Status)
    }
