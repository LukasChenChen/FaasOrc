...
class SimpleHTTPRequestHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        logging.info("receive Get")
        self.send_response(200)
        self.end_headers()
        self.wfile.write(b'Hello, world!')

    def do_POST(self):
        logging.info("receive Post")
    ...
    #        logging.warning(request)
        ...
        message_1 = threading.currentThread().getName()
        self.wfile.write(message_1.encode('utf-8'))
        self.wfile.write(b'\n')
...
def send_request(request):
    ...
    global REQ_COUNT
    runtime = 0
    if "IsNotPredicted" in request:
        if request['IsNotPredicted'] == False:
            time.sleep(8)
    starttime = time.time()
    while True:
    ...
    if "IsNotPredicted" in request:
        result = [{'time': request['ArriveTime'], 'ID': request['ID'],'type': request['Function']['Type'], 'linkDelay':link_delay, 'processing_delay': processing_delay, 'runtime':runtime, 'total_delay':total_delay, 'cold_start':request['IsColdStart'],'IsNotPredicted':request['IsNotPredicted']}]
